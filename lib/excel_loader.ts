        ...
        scoreData.forEach(row => {
            ...
            if (name) {
                ...
                Object.keys(row).forEach(key => {
                    if (key.startsWith('단어시험') || key.startsWith('독해단어')) {
                        // week1-1 -> 1, week1-2 -> 2, week2-1 -> 3, week2-2 -> 4 logic or simple name matching
                        if (key.includes('week1-1') || key.endsWith('1')) vocabScores['vocab1'] = parseScore(row[key]);
                        else if (key.includes('week1-2') || key.endsWith('2')) vocabScores['vocab2'] = parseScore(row[key]);
                        else if (key.includes('week2-1') || key.endsWith('3')) vocabScores['vocab3'] = parseScore(row[key]);
                        else if (key.includes('week2-2') || key.endsWith('4')) vocabScores['vocab4'] = parseScore(row[key]);
                    }
                });

                const vocab3Key = Object.keys(row).find(k => k.replace(/\s+/g, '') === '단어시험week2-1') || '독해단어3';
                const vocab4Key = Object.keys(row).find(k => k.replace(/\s+/g, '') === '단어시험week2-2') || '독해단어4';
                const vocab5Key = Object.keys(row).find(k => k.replace(/\s+/g, '') === '단어시험week3-1') || '독해단어5';
                ...
                scoresMap.set(name, {
                    ...
                    class: row['반']?.toString().trim() || '',
                    school: row['학교']?.toString().trim() || '',
                    vocab1: vocabScores['vocab1'] ?? parseScore(row['독해단어1']) ?? parseScore(row['단어시험week1-1']),
                    vocab2: vocabScores['vocab2'] ?? parseScore(row['독해단어2']) ?? parseScore(row['단어시험week1-2']),
                    vocab3: vocabScores['vocab3'] ?? parseScore(row[vocab3Key]) ?? parseScore(row['단어시험week2-1']),
                    vocab4: vocabScores['vocab4'] ?? parseScore(row[vocab4Key]) ?? parseScore(row['단어시험week2-2']),
                    vocab5: vocabScores['vocab5'] ?? parseScore(row[vocab5Key]) ?? parseScore(row['단어시험week3-1']),
    ...
    allStudentNames.forEach(name => {
        ...
        const history: WeeklyReportData = {
            ...
            vocab: {
                ...
                tiedCount: 0,
                title: `${weekId.split('-W')[1]}주차 독해단어 (Vocabulary)`
            },
            grammarTheory: {
